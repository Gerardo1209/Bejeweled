...
public class Graficos extends JComponent {
    ...
    public final static int ALTO = 540, ANCHO = 640; //Alto y ancho del componente (lienzo), sera del tamano de la ventana (JFrame) principal
    public final static int DIMENSION_CUADRADOS_TABLERO = 44;
    public final static int LUGAR_INICIO_TABLERO_X = DIMENSION_CUADRADOS_TABLERO * 5, LUGAR_INICIO_TABLERO_Y = DIMENSION_CUADRADOS_TABLERO * 2;
    ...
    private int columnaClickeada = -1, filaClickeada = -1; //Se inicializa en -1 para que no se coloree el contorno de blanco por defecto.
    ...
    private void inicializarGemas() { //Crea todas las instancias de las gemas
        ...
        for(int i = 0; i < FILAS_TABLERO; i++){
            for(int j = 0; j < COLUMNAS_TABLERO; j++){
                ...
                /*if(i==2&&j==3){
                    this.matrizGemas[i][j].crearGemaHiperCubo();
                }
                if(i==3&&j==3){
                    this.matrizGemas[i][j].crearGemaEspecial();
                }*/
            }
        }
        ...
        if(verificar){
            ...
            for(int i = 0; i < FILAS_TABLERO; i++){
                for(int j = 0; j < COLUMNAS_TABLERO; j++) {
                    ...
                    int abajo = inicio.getAbajo(), arriba = inicio.getArriba(), izquierda = inicio.getIzquierda(), derecha = inicio.getDerecha();
            ...
            for(int i = 0; i < FILAS_TABLERO; i++){
                for(int j = 0; j < COLUMNAS_TABLERO; j++){
                    if(matrizGemas[i][j].getId()==0){
                        ...
                        puntos += ((inicio.analizaGemaAntes(matrizGemas[i][j],null,g,explosionCreada,explosion)) * 50);
                        int abajo = inicio.getAbajo(), arriba = inicio.getArriba(), izquierda = inicio.getIzquierda(), derecha = inicio.getDerecha();
                        if(matrizGemas[i][j].getId() == 0){
                            bajaRecursiva(i, j,g,true);
                            int v = 1;
    ...
    private void verificarGemasSeleccionadas(Graphics g){
        Scanner inicio = new Scanner(matrizGemas);
        if(matrizGemas[auxFilaClikeada][auxColumnaClickeada].getId() == 15 || matrizGemas[filaClickeada][columnaClickeada].getId() == 15){
            dobleHyper = false;
            int idEliminar = 0;
            puntos += 50;
            ...
            }else if(matrizGemas[auxFilaClikeada][auxColumnaClickeada].getId() == 15 && matrizGemas[filaClickeada][columnaClickeada].getId() == 15){
                matrizGemas[auxFilaClikeada][auxColumnaClickeada].setId(0);
                matrizGemas[filaClickeada][columnaClickeada].setId(0);
                //bajaRecursiva(filaClickeada, columnaClickeada,g,true);
                //bajaRecursiva(auxFilaClikeada, auxColumnaClickeada,g,true);
            ...
            for(int i=0; i<FILAS_TABLERO; i++){
                for(int j=0; j<COLUMNAS_TABLERO; j++){
                    if((matrizGemas[i][j].getId() == idEliminar || matrizGemas[i][j].getId() == (idEliminar+7) || matrizGemas[i][j].getId() == (idEliminar-7)) && idEliminar!= 0){
                        if(matrizGemas[i][j].getId()>7){
                            puntos += (new Scanner(matrizGemas).eliminarGemaEspecial(matrizGemas[i][j], g, (matrizGemas[i][j].getId()-7))*50);
                            explosion.setSonidoUnaVez(true);
                        }
                        matrizGemas[i][j].setId(0);
                        g.drawImage(explosionIcon.getImage(),matrizGemas[i][j].getCoordenadaX_Inicio_Cuadrado(),matrizGemas[i][j].getCoordenadaY_Inicio_Cuadrado(),null);
                        bajaRecursiva(i, j,g,true);
                        puntos += 50;
                    }
                    if(dobleHyper){
            ...
            if(scanner_gemas.analizaGema(matrizGemas[auxFilaClikeada][auxColumnaClickeada])){
                puntos += ((inicio.analizaGemaAntes(matrizGemas[auxFilaClikeada][auxColumnaClickeada],null,g,explosionCreada,explosion)) * 50);
            ...
            if(scanner_gemas.analizaGema(matrizGemas[filaClickeada][columnaClickeada])){
                puntos += ((inicio.analizaGemaAntes(matrizGemas[filaClickeada][columnaClickeada],null,g,explosionCreada,explosion)) * 50);
                int abajo = inicio.getAbajo(), arriba = inicio.getArriba(), izquierda = inicio.getIzquierda(), derecha = inicio.getDerecha();
    ...
    public void iniciaMouse(){
        addMouseMotionListener(new MouseMotionAdapter() {
            ...
            public void mouseMoved(MouseEvent mouseMovimiento) {
                ...
                if(((coordenadaCursorX >= LUGAR_INICIO_TABLERO_X) && (coordenadaCursorX <= (LUGAR_INICIO_TABLERO_X + (DIMENSION_CUADRADOS_TABLERO * 9))))
                        && ((coordenadaCursorY >= LUGAR_INICIO_TABLERO_Y) && (coordenadaCursorY <= (LUGAR_INICIO_TABLERO_Y + (DIMENSION_CUADRADOS_TABLERO * 9))))) {
                    if(columnaPorClickear!=getColumnaGema(coordenadaCursorX) || filaPorClickear!=getFilaGema(coordenadaCursorY)){
            ...
            public void mouseClicked(MouseEvent mousePulsado) {
                if(!eliminacion && !pausa && !movimientoRetorno){
                    if ((((columnaPorClickear == columnaClickeada + 1 || columnaPorClickear == columnaClickeada - 1)
                            && filaPorClickear == filaClickeada) || ((filaPorClickear == filaClickeada + 1 || 
                            filaPorClickear == filaClickeada - 1)) && columnaPorClickear == columnaClickeada) &&
                            (filaPorClickear != -1 && columnaPorClickear!=-1 && filaClickeada!=-1 && columnaClickeada != -1)) {
                        eliminacion = false;
                        ...
                        if(!scanner_gemas.analizaGema(matrizGemas[filaPorClickear][columnaPorClickear])&&
                            !scanner_gemas.analizaGema(matrizGemas[filaClickeada][columnaClickeada])){
                                if(matrizGemas[filaPorClickear][columnaPorClickear].getId() == 15 || matrizGemas[filaClickeada][columnaClickeada].getId() == 15){
    ...
    public int getColumnaGema(int coordenadaCursorX) {
        for(int i = 0; i < FILAS_TABLERO; i++){
            for(int j = 0; j < COLUMNAS_TABLERO; j++){
                if(coordenadaCursorX >= matrizGemas[i][j].getCoordenadaX_Inicio_Cuadrado() && coordenadaCursorX <= matrizGemas[i][j].getCoordenadaX_Fin_Cuadrado()) {
    ...
    public int getFilaGema(int coordenadaCursorY) {
        for(int i = 0; i < FILAS_TABLERO; i++){
            for(int j = 0; j < COLUMNAS_TABLERO; j++){
                if(coordenadaCursorY >= matrizGemas[i][j].getCoordenadaY_Inicio_Cuadrado() && coordenadaCursorY <= matrizGemas[i][j].getCoordenadaY_Fin_Cuadrado()){
    ...
    private void crearTablero(Graphics g) {
        ...
        for(int i = 0; i < FILAS_TABLERO; i++){
            for(int j = 0; j < COLUMNAS_TABLERO; j++){
                ...
                //if(matrizGemas[i][j].getPosXNueva()!=matrizGemas[i][j].getPosXAnterior() || matrizGemas[i][j].getPosYNueva()!=matrizGemas[i][j].getPosYAnterior()){
                //    movimientoGema(g,i,j);
                //}else{
                    matrizGemas[i][j].setFilaYColumnaActual(i, j); //Obtiene valores de la matriz logica para el objeto en cuestion
                    //Manda las posiciones en las que se encuentra la gema, se utilizara junto al mouse listener para moverlas
                    matrizGemas[i][j].setCoordenadas_Tablero_En_X(LUGAR_INICIO_TABLERO_X + (DIMENSION_CUADRADOS_TABLERO * j)); //Obtiene valores en x de la matriz de pixeles
                    matrizGemas[i][j].setCoordenadas_Tablero_En_Y(LUGAR_INICIO_TABLERO_Y + (DIMENSION_CUADRADOS_TABLERO * i)); //Obtiene valores en y de la matriz de pixeles
    ...
    /*public void movimientoGema(Graphics g, int i, int j){
        int XPosInicial1 = LUGAR_INICIO_TABLERO_X + (DIMENSION_CUADRADOS_TABLERO * matrizGemas[i][j].getPosXAnterior());
        int YPosInicial1 = LUGAR_INICIO_TABLERO_Y + (DIMENSION_CUADRADOS_TABLERO * matrizGemas[i][j].getPosYAnterior());
        int XPosInicial2 = LUGAR_INICIO_TABLERO_X + (DIMENSION_CUADRADOS_TABLERO * matrizGemas[i][j].getPosXNueva());
        int YPosInicial2 = LUGAR_INICIO_TABLERO_Y + (DIMENSION_CUADRADOS_TABLERO * matrizGemas[i][j].getPosYNueva());
        if(matrizGemas[i][j].getPosXAnterior()<matrizGemas[i][j].getPosXNueva()){
            matrizGemas[matrizGemas[i][j].getPosYAnterior()][matrizGemas[i][j].getPosXAnterior()].dibujarGemaEnTableroMovimiento(g,XPosInicial2-(int)aumentoMovimiento,YPosInicial2);
            matrizGemas[i][j].dibujarGemaEnTableroMovimiento(g,XPosInicial1+ (int)aumentoMovimiento,YPosInicial1);
        }else if(matrizGemas[i][j].getPosXAnterior()>matrizGemas[i][j].getPosXNueva()){
            matrizGemas[matrizGemas[i][j].getPosYAnterior()][matrizGemas[i][j].getPosXAnterior()].dibujarGemaEnTableroMovimiento(g,XPosInicial2+(int)aumentoMovimiento,YPosInicial2);
            matrizGemas[i][j].dibujarGemaEnTableroMovimiento(g,XPosInicial1-(int)aumentoMovimiento,YPosInicial1);
        }
        if(matrizGemas[i][j].getPosYAnterior()<matrizGemas[i][j].getPosYNueva()){
            matrizGemas[matrizGemas[i][j].getPosYAnterior()][matrizGemas[i][j].getPosXAnterior()].dibujarGemaEnTableroMovimiento(g,XPosInicial2,YPosInicial2-(int)aumentoMovimiento);
            matrizGemas[i][j].dibujarGemaEnTableroMovimiento(g,XPosInicial1,YPosInicial1+(int)aumentoMovimiento);
        }else if(matrizGemas[i][j].getPosYAnterior()<matrizGemas[i][j].getPosYNueva()){
            matrizGemas[matrizGemas[i][j].getPosYAnterior()][matrizGemas[i][j].getPosXAnterior()].dibujarGemaEnTableroMovimiento(g,XPosInicial2,YPosInicial2+(int)aumentoMovimiento);
            matrizGemas[i][j].dibujarGemaEnTableroMovimiento(g,XPosInicial1,YPosInicial1-(int)aumentoMovimiento);
        }
        if(movimientoRetorno && primeroListo){
            XPosInicial1 = LUGAR_INICIO_TABLERO_X + (DIMENSION_CUADRADOS_TABLERO * matrizGemas[i][j].getPosXNueva());
            YPosInicial1 = LUGAR_INICIO_TABLERO_Y + (DIMENSION_CUADRADOS_TABLERO * matrizGemas[i][j].getPosYNueva());
            XPosInicial2 = LUGAR_INICIO_TABLERO_X + (DIMENSION_CUADRADOS_TABLERO * matrizGemas[i][j].getPosXAnterior());
            YPosInicial2 = LUGAR_INICIO_TABLERO_Y + (DIMENSION_CUADRADOS_TABLERO * matrizGemas[i][j].getPosYAnterior());
            if(matrizGemas[i][j].getPosXAnterior()<matrizGemas[i][j].getPosXNueva()){
                matrizGemas[matrizGemas[i][j].getPosYAnterior()][matrizGemas[i][j].getPosXAnterior()].dibujarGemaEnTableroMovimiento(g,XPosInicial2+(int)aumentoMovimiento,YPosInicial2);
                matrizGemas[i][j].dibujarGemaEnTableroMovimiento(g,XPosInicial1-(int)aumentoMovimiento,YPosInicial1);
            }else if(matrizGemas[i][j].getPosXAnterior()>matrizGemas[i][j].getPosXNueva()){
    ...
    private void bajaRecursiva(int i, int j){
        if(i == 0){
            ...
            matrizGemas[i][j].setCoordenadas_Tablero_En_X(LUGAR_INICIO_TABLERO_X + (DIMENSION_CUADRADOS_TABLERO * j)); //Obtiene valores en x de la matriz de pixeles
            matrizGemas[i][j].setCoordenadas_Tablero_En_Y(LUGAR_INICIO_TABLERO_Y + (DIMENSION_CUADRADOS_TABLERO * (i-1))); //Obtiene valores en y de la matriz de pixeles
            return;
        }else{  
    ...
    private void bajaRecursiva(int i, int j, Graphics g, boolean first){
        if(i == 0){
            if(!first){
                g.drawImage(flechaIcon.getImage(),matrizGemas[i][j].getCoordenadaX_Inicio_Cuadrado(),matrizGemas[i][j].getCoordenadaY_Inicio_Cuadrado(),null);
            }
            matrizGemas[i][j] = new Gemas();
            matrizGemas[i][j].setPosYNueva(i);
            matrizGemas[i][j].setPosXNueva(j);
    ...
    private void pintarCuadroPorSeleccionar(Graphics g) {
        if(((coordenadaCursorX >= LUGAR_INICIO_TABLERO_X) && (coordenadaCursorX <= (LUGAR_INICIO_TABLERO_X + (DIMENSION_CUADRADOS_TABLERO * 9))))
            ...
            g2.drawLine(matrizGemas[filaPorClickear][columnaPorClickear].getCoordenadaX_Inicio_Cuadrado(),
                    ...
                    matrizGemas[filaPorClickear][columnaPorClickear].getCoordenadaX_Fin_Cuadrado() - (DIMENSION_CUADRADOS_TABLERO / 2),
                    matrizGemas[filaPorClickear][columnaPorClickear].getCoordenadaY_Inicio_Cuadrado());

            g2.drawLine(matrizGemas[filaPorClickear][columnaPorClickear].getCoordenadaX_Inicio_Cuadrado(),
                    matrizGemas[filaPorClickear][columnaPorClickear].getCoordenadaY_Inicio_Cuadrado(),
                    matrizGemas[filaPorClickear][columnaPorClickear].getCoordenadaX_Inicio_Cuadrado(),
                    matrizGemas[filaPorClickear][columnaPorClickear].getCoordenadaY_Fin_Cuadrado() - (DIMENSION_CUADRADOS_TABLERO / 2));
            ...
            g2.drawLine(matrizGemas[filaPorClickear][columnaPorClickear].getCoordenadaX_Fin_Cuadrado(),
                    ...
                    matrizGemas[filaPorClickear][columnaPorClickear].getCoordenadaY_Inicio_Cuadrado() + (DIMENSION_CUADRADOS_TABLERO / 2));

            g2.drawLine(matrizGemas[filaPorClickear][columnaPorClickear].getCoordenadaX_Fin_Cuadrado(),
                    matrizGemas[filaPorClickear][columnaPorClickear].getCoordenadaY_Fin_Cuadrado(),
                    matrizGemas[filaPorClickear][columnaPorClickear].getCoordenadaX_Inicio_Cuadrado() + (DIMENSION_CUADRADOS_TABLERO / 2),
                    matrizGemas[filaPorClickear][columnaPorClickear].getCoordenadaY_Fin_Cuadrado());
        ...
        if(((coordenadaCursorX >= LUGAR_INICIO_TABLERO_X) && (coordenadaCursorX <= (LUGAR_INICIO_TABLERO_X + (DIMENSION_CUADRADOS_TABLERO * 9))))
            ...
            if (columnaClickeada != -1 && filaClickeada != -1) {
                g2.drawLine(matrizGemas[filaClickeada][columnaClickeada].getCoordenadaX_Inicio_Cuadrado(),
                        ...
                        matrizGemas[filaClickeada][columnaClickeada].getCoordenadaX_Fin_Cuadrado() - (DIMENSION_CUADRADOS_TABLERO / 2),
                        matrizGemas[filaClickeada][columnaClickeada].getCoordenadaY_Inicio_Cuadrado());
                g2.drawLine(matrizGemas[filaClickeada][columnaClickeada].getCoordenadaX_Inicio_Cuadrado(),
                        matrizGemas[filaClickeada][columnaClickeada].getCoordenadaY_Inicio_Cuadrado(),
                        matrizGemas[filaClickeada][columnaClickeada].getCoordenadaX_Inicio_Cuadrado(),
                        matrizGemas[filaClickeada][columnaClickeada].getCoordenadaY_Fin_Cuadrado() - (DIMENSION_CUADRADOS_TABLERO / 2));
                g2.drawLine(matrizGemas[filaClickeada][columnaClickeada].getCoordenadaX_Fin_Cuadrado(),
                        ...
                        matrizGemas[filaClickeada][columnaClickeada].getCoordenadaY_Inicio_Cuadrado() + (DIMENSION_CUADRADOS_TABLERO / 2));
                g2.drawLine(matrizGemas[filaClickeada][columnaClickeada].getCoordenadaX_Fin_Cuadrado(),
                        matrizGemas[filaClickeada][columnaClickeada].getCoordenadaY_Fin_Cuadrado(),
                        matrizGemas[filaClickeada][columnaClickeada].getCoordenadaX_Inicio_Cuadrado() + (DIMENSION_CUADRADOS_TABLERO / 2),
                        matrizGemas[filaClickeada][columnaClickeada].getCoordenadaY_Fin_Cuadrado());
            }
        }
    }
    ...
    private void inicializarTimer() {
        timer = new Reloj(DIMENSION_CUADRADOS_TABLERO * 8, DIMENSION_CUADRADOS_TABLERO); //Crea el reloj y mostrará el tiempo en la posicion dada
